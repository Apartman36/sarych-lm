...
def generate_text(
    *,
    ...
    model = SarychLM(model_config_from_dict(config)).to(resolved_device)
    load_checkpoint(
        checkpoint_path,
        model=model,
        optimizer=None,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate text with a SARYCH-LM v0.3 checkpoint.")
